...
import { useAssetResult, useSnapshots, useUpdateAsset } from "../service/hook"
...
const AssetDetail = () => {
    ...
    const { data, isLoading } = useAssetResult(assetId!)
    ...
    return (
        <div className="container flex flex-col items-center">
            <AppBar
                ...
                title={data.name}
            />
            <AssetIcon asset={data} className="w-14 h-14 mt-5" />
...
const Balance: FC<{ data: AssetSchema } & HTMLAttributes<HTMLAnchorElement>> = ({ data, className }) => {
    const balance = useMemo(() => bigMul(data.balance, data.price_usd, data.fiat?.rate ?? 0), [data.balance, data.price_usd, data.fiat?.rate])
    ...
    return <div className={`flex flex-col gap-1 ${className}`}>
        <div className="flex gap-1 flex-wrap justify-center">
            <FormatNumber
                value={data.balance}
                precision={'crypto'}
                className="font-semibold text-3xl break-all text-center"
            />
...
const List = () => {
    const { assetId } = useParams()
    const { data, fetchNextPage } = useSnapshots({ assetId: assetId! })
    ...
    return <div className="w-full">
        ...
        <InfiniteScroll onIntersect={fetchNextPage}>
            {list?.map(item => <ListItem key={item.snapshot_id} data={item} />)}
        </InfiniteScroll>
    </div>
}
...
const ListItem: FC<{ data: SnapshotSchema }> = ({ data }) => {
    ...
    return (
        <Button to={{}} className="h-[72px] p-4 w-full flex gap-3">
            ...
            <div className="flex gap-1 text-sm flex-shrink-0">
                {
                    isPositive ? <FormatNumber value={data.amount} precision={'crypto'} leading="+" className="text-green-500 font-semibold" />
                        : <FormatNumber value={data.amount} precision={'crypto'} className="text-red-500 font-semibold" />
                }
...
const Date: FC<{ date: string }> = ({ date }) => {
    const dateText = useMemo(() => dayjs(date).format('DD/MM/YYYY'), [date])
    return <>{dateText}</>
}

const ListHeader = () => {
    const [t] = useTranslation()
    return <div className="w-full h-10 py-2 px-4 font-semibold">{t('transactions')}</div>
