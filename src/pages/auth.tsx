...
const Auth = () => {
  ...
  useEffect(() => {
    ...
    const login = async (code: string) => {
      try {
        const { access_token, scope } = await mixinRequest.post<any, { access_token: string; scope: string }>('/oauth/token', {
        ...
        if (!scope || !scope.includes('ASSETS:READ') || !scope.includes('SNAPSHOTS:READ')) {
  ...
  return (
    <div className="container w-screen h-screen flex flex-col pt-5 mb-8 items-center">
      <div className="relative">
        <img className="w-[360px] h-[360px] object-cover" src={authBackground} />
        <img className="w-[96px] h-[96px] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" src={logo} />
      </div>
